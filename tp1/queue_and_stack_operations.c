...
int pile_variable_empiler(pile_variable_t *pile, element_t element) {
    if (pile->sommet >= pile->taille - 1) { // If the stack is full
...
int pile_variable_depiler(pile_variable_t *pile, element_t *p_element) {
    ...
    *p_element = pile->pile[pile->sommet--]; // Pop the element
    ...
    if (pile->sommet < pile->taille - PAS_ALLOCATION) { // If the number of elements in the stack is smaller than the size of the stack by PAS_ALLOCATION
...
int file_fixe_defiler(file_fixe_t *file, element_t *p_element) {
    ...
    *p_element = file->file[file->head++];
    ...
    if (file->head == file->tail) { // The queue is empty
        file->head = -1;
        file->tail = 0;
    }
